    ...
    with open(js_path, "r", encoding="utf-8", errors="ignore") as f:
        ...
        for url in scanned_urls:
            if url not in api_spec:
                api_spec[url] = {"name": "scanned", "method": "GET", "body_fields": set()}

        closure_urls = resolve_closure_urls(content)
        for url, name in closure_urls.items():
            if url not in api_spec:
                api_spec[url] = {"name": name, "method": "GET", "body_fields": set()}

        dynamic_urls = parse_dynamic_urls(content)
        for url in dynamic_urls:
            if url not in api_spec:
                api_spec[url] = {"name": "dynamic_resolved", "method": "GET", "body_fields": set()}
...
with open(final_output, "w", encoding="utf-8") as out:
    ...
    for url, info in sorted(api_spec.items()):
        ...
        if info['method'] == "POST":
            ...
            clean_body = [f for f in info['body_fields'] if not is_noise(f)]

            if clean_body:
                for f in sorted(clean_body):
                    if f != info['name']:
                        out.write(f'         "{f}": "value",\n')
            else:
